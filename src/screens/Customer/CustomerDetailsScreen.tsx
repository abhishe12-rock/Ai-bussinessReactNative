...
export default function CustomerDetailsScreen() {
  ...
  return (
    <View style={styles.flex}>
      {/* HEADER — Cupertino-style back button (chevron + label), inline with actions */}
      <View style={styles.header}>
        <TouchableOpacity
        ...
        <View style={styles.headerActions}>
          <TouchableOpacity style={styles.headerActionBtn} onPress={() => {}}>
            <Icon name="call" color={AppColors.primary} size={18} />
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.headerActionBtn}
            onPress={() => navigation.navigate('EditCustomer', { customer })}
          >
            <Icon name="edit" color={AppColors.textSecondary} size={18} />
          </TouchableOpacity>
        </View>
      </View>
      ...
      {repairs.map((r) => (
        <View key={r.id} style={styles.repairCard}>
          ...
          <View style={styles.repairLineRow}>
            <Icon name="smartphone" color={AppColors.textMuted} size={14} />
            <Text style={styles.repairLine}>{r.device}</Text>
          </View>
          <View style={styles.repairLineRow}>
            <Icon name="build" color={AppColors.textMuted} size={14} />
            <Text style={styles.repairLine}>{r.problemDescription}</Text>
          </View>
          <Text style={styles.repairDate}>{r.createdAt.toLocaleDateString()}</Text>
          {r.assignedToName && (
            <View style={styles.repairLineRow}>
              <Icon name="engineering" color={AppColors.textMuted} size={14} />
              <Text style={styles.repairTech}>{r.assignedToName}</Text>
            </View>
          )}
        </View>
      ))}
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 52, height: 52, borderRadius: 26, backgroundColor: AppColors.primarySoft, alignItems: 'center', justifyContent: 'center', marginRight: 14 },
  avatarText: { color: AppColors.primary, fontWeight: '700', fontSize: 16 },
  name: { color: AppColors.textPrimary, fontSize: 17, fontWeight: '700' },
  subRow: { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 3 },
  sub: { color: AppColors.textSecondary, fontSize: 12.5 },
  ...
  statusPill: { paddingHorizontal: 9, paddingVertical: 4, borderRadius: 20 },
  statusText: { fontSize: 10.5, fontWeight: '700' },
  repairLineRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 6 },
  repairLine: { color: AppColors.textSecondary, fontSize: 13 },
  repairDate: { color: AppColors.textMuted, fontSize: 11.5, marginTop: 6 },
  repairTech: { color: AppColors.textSecondary, fontSize: 12, fontWeight: '600' },
});